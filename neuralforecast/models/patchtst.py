    ...
    **References:**<br>
    ...
    def __init__(
        ...
        **trainer_kwargs
        ...
        # Historical exogenous: fuse with y at each timestep before patching
        if self.hist_exog_size > 0:
            self.hist_exog_merge = nn.Linear(1 + self.hist_exog_size, 1)

        # Static exogenous: project to forecast bias
        if self.stat_exog_size > 0:
            self.stat_exog_head = nn.Sequential(
                nn.Linear(self.stat_exog_size, hidden_size),
                nn.ReLU(),
                nn.Linear(hidden_size, h * c_out),
            )

    def forward(self, windows_batch):  # x: [batch, input_size]

        # Parse windows_batch
        x = windows_batch["insample_y"]  # [B, input_size, 1]
        hist_exog = windows_batch["hist_exog"]  # [B, input_size, hist_exog_size]
        stat_exog = windows_batch["stat_exog"]  # [B, stat_exog_size]

        # Fuse historical exogenous with y at each timestep
        if self.hist_exog_size > 0:
            x = torch.cat([x, hist_exog], dim=-1)  # [B, input_size, 1 + hist_exog_size]
            x = self.hist_exog_merge(x)  # [B, input_size, 1]

        x = x.permute(0, 2, 1)  # x: [Batch, 1, input_size]
        x = self.model(x)
        forecast = x.reshape(x.shape[0], self.h, -1)  # x: [Batch, h, c_out]

        # Add static exogenous bias
        if self.stat_exog_size > 0:
            stat_bias = self.stat_exog_head(stat_exog)  # [B, h * c_out]
            stat_bias = stat_bias.reshape(
                -1, self.h, self.loss.outputsize_multiplier
            )  # [B, h, c_out]
            forecast = forecast + stat_bias
