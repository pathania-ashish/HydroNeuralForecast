    ...
    *Parameters:*<br>
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        # Embedding
        # Calculate total exogenous sizes
        enc_exog_input_size = self.futr_exog_size + self.hist_exog_size + self.stat_exog_size
        dec_exog_input_size = self.futr_exog_size + self.stat_exog_size  # No hist_exog in decoder

        self.enc_embedding = DataEmbedding(
            c_in=self.enc_in,
            exog_input_size=enc_exog_input_size,
        ...
        self.dec_embedding = DataEmbedding(
            self.dec_in,
            exog_input_size=dec_exog_input_size,
            hidden_size=hidden_size,
            pos_embedding=True,
            dropout=dropout,
        )
    ...
    def forward(self, windows_batch):
        ...
        # ========== Prepare Encoder Exogenous Features ==========
        x_mark_enc_list = []

        # Historical exogenous for encoder window [B, L, H]
        if self.hist_exog_size > 0:
            x_mark_enc_list.append(hist_exog)

        # Future exogenous for encoder window [B, L, F]
        if self.futr_exog_size > 0:
            x_mark_enc_list.append(futr_exog[:, : self.input_size, :])

        # Static exogenous - broadcast to encoder length [B, S] -> [B, L, S]
        if self.stat_exog_size > 0:
            stat_exog_enc = stat_exog.unsqueeze(1).repeat(1, self.input_size, 1)
            x_mark_enc_list.append(stat_exog_enc)

        # Concatenate all encoder exogenous features
        x_mark_enc = torch.cat(x_mark_enc_list, dim=-1) if len(x_mark_enc_list) > 0 else None

        # ========== Prepare Decoder Exogenous Features ==========
        x_mark_dec_list = []
        decoder_len = self.label_len + self.h

        # Future exogenous for decoder window [B, label_len + h, F]
        if self.futr_exog_size > 0:
            x_mark_dec_list.append(futr_exog[:, -decoder_len:, :])

        # Static exogenous - broadcast to decoder length [B, S] -> [B, label_len + h, S]
        if self.stat_exog_size > 0:
            stat_exog_dec = stat_exog.unsqueeze(1).repeat(1, decoder_len, 1)
            x_mark_dec_list.append(stat_exog_dec)

        # Concatenate all decoder exogenous features
        x_mark_dec = torch.cat(x_mark_dec_list, dim=-1) if len(x_mark_dec_list) > 0 else None
